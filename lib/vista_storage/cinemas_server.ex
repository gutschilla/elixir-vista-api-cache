defmodule VistaStorage.CinemasServer do

  @moduledoc ~S"""
  This is a copy of VistaStorage.SessionsServer with all occurences of "session"
  replaced with "cinema". Also register_item_for_reload was changed to just
  ...
  def start_link_for(module, opts) do
    default_state  = State.new(:cinemas_server)
    name =
      opts
  ...
  def handle_cast({:updated, type = :full_reload}, state) do
    apply_on_change(state.on_change, {type, :cinemas})
    log_telemetry(:full_reload, %{count: length(state.items)})
    Logger.debug "update: full_reload - having #{length(state.items)} cinemas, now"
    {:noreply, state}
  end
  ...
  def handle(:full_reload, state = %State{timer: old_timer_ref, ttl: ttl}) do
    Logger.info("reload cinemas")
    {:ok, cinemas} = client().get_cinemas() # errors shall crash, will be restarted
    new_timer_ref = restart_timer(old_timer_ref, :full_reload, 1000 * ttl)
    notify_updated(:full_reload)
    %State{state | items: cinemas, timer: new_timer_ref}
  end

  @type state     :: State.t()
  @type cinema_id :: binary()
  @type index     :: integer()
  @type item      :: Item.t()
  @type items     :: [item()]
  ...
  @spec get_index_for_id([items], cinema_id) :: index
  def get_index_for_id(items, id) when is_binary(id) do
    Enum.find_index(items, fn %Cinema{id: ^id} -> true; _ -> false end)
  end
  ...
  def apply_on_change({m, f, a}, args) do
    apply(m, f, [a, args])
  end
  ...
  def log_telemetry(what, metrics = %{}) when what in [:updated, :full_reload, :removed] do
    ...
    apply(module, function, [[:storage, :cinemas, what, :done], metrics, %{}])
